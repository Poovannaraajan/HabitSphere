...
const MealSection = ({ dietaryRestrictions = [] }) => {
  ...
  // ============= API FUNCTIONS =============
  // Parse user input into individual food items
 ...
 const searchFood = async (foodName) => {
  try {
    ...
    if (Array.isArray(data.foods?.food)) {
      ...
      console.log('✅ Found array at: data.foods.food');
    ...
    console.log('🔍 Returning', foodArray.length, 'foods');
    if (foodArray.length > 0) {
      console.log('First food:', foodArray);
    }
 ...
 const getFoodDetails = async (foodId) => {
  try {
    ...
    console.log('  - data.food:', data.food ? 'exists' : 'missing');
    console.log('  - servings:', data.food?.servings ? 'exists' : 'missing');
  ...
  const analyzeHealthInsights = () => {
    ...
    if (dailyNutrition.protein > DAILY_RECOMMENDATIONS.protein.max) {
      insights.warnings.push('High protein intake can lead u to death');
  ...
  const handleLogFood = async () => {
    ...
    try {
      ...
      for (const foodName of foodNames) {
        ...
        // ✅ FIX 1: Get FIRST element from array
        ...
        if (!foodDetails || !foodDetails.food || !foodDetails.food.servings) {  // ✅ CORRECT
  console.warn('⚠️ No servings data');
  continue;
}

        // ✅ FIX 2: Get FIRST serving element
        const serving = Array.isArray(foodDetails.food.servings.serving)  // ✅ CORRECT
...
console.log('📊 Serving:', serving);
      ...
      // Step 4: Calculate total nutrition ONCE
      let totalNutrition = {
        calories: 0,
        protein: 0,
        carbs: 0,
      ...
      <div className="flex-1">
        ...
        {healthInsights.recommendations.map((rec, idx) => (
          <p key={idx} className="text-sm text-gray-600 mb-1">
            {healthInsights.overall === 'excellent' ? '💡' : '💡'} {rec}
          </p>
        ))}
      </div>
    </div>
